...
def unpool(inputs, fac=2, data_format='channels_last'):
    ...
    inputs = tf.compat.v1.image.resize_bilinear(
        inputs, size=[tf.shape(inputs)[1]*int(fac),  tf.shape(inputs)[2]*int(fac)])
...
def conv_layer(input, output_dep, filter_size, is_training=True, data_format='channels_first'):
    input = tf.layers.conv2d(input, 256, 1, kernel_initializer=initializers.variance_scaling_initializer(
...
def model(inputs, data_format='channels_first', is_training=True):
    ...
    with tf.variable_scope('fpn_module'):
        ...
        thresh_map = tf.sigmoid(unpool(thresh_map, 4, data_format=data_format)
                                )  # thresh_map (n,1,h,w) seg_maps(n,c,h,w)
        # the thresh_map is (n,1,h,w),need squeeze
        thresh_map = thresh_map[:, 0, :, :]  # the thersh map only 1 channel
        # differentiable binalization
        # the last channel is the smallest kernel
        binary_map = tf.math.reciprocal(1 + tf.exp(-config['k'] * (seg_maps[:, -1, :, :]-thresh_map)))
