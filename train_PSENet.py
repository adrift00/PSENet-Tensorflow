...
def tower_loss(scope, images, labels, kernels, training_mask, gt_thresh, thresh_mask):
    ...
    tf.compat.v1.summary.image(re.sub('%s_[0-9]*/' % 'TOWER', '', 'train_mask'), tf.expand_dims(training_mask, -1))
    tf.compat.v1.summary.image(re.sub('%s_[0-9]*/' % 'TOWER', '%d_' %
                                      i, 'thresh_map'), tf.expand_dims(thresh_map[:, :, :], -1))
    tf.compat.v1.summary.image(re.sub('%s_[0-9]*/' % 'TOWER', '%d_' %
                                      i, 'binary_map'), tf.expand_dims(binary_map[:, :, :], -1))
    tf.compat.v1.summary.image(re.sub('%s_[0-9]*/' % 'TOWER', '%d_' %
...
def main(argv=None):
    ...
    tf.compat.v1.summary.scalar('total_loss', loss_sum/int(num_gpu))
